...
let play=()=>{
    let sigml=words.filter(word=>word.name===currentword)[0].value
    parent.appletframe.playText(sigml)
    document.querySelector("#comment").value=currentword
    //console.log(sigml)
}

document.querySelector("#submit").addEventListener('click',(e)=>{
    let comment=document.querySelector("#comment").value
    let cmt={name:currentword,comment:comment}
    console.log(cmt)

    const url = 'http://localhost:8080/posthere';

    if (confirm(cmt.name+"\n"+cmt.comment)) {

    fetch(url, {
        method: 'POST',
        body: JSON.stringify({name:cmt.name,comment:cmt.comment}),
        headers: {
...
fetch('http://localhost:8080/getfiles')
  ...
  .then(
    data => {
        words=data.filelist
        last_updat=data.last
        console.log(data)
        let ihtml=``;
        let ct=0
        data.filelist.map(sign=>{
            
           ihtml+=`<li id="id${ct}">${sign.name}</li>` 
           ct+=1
        })

        document.querySelector('#wordlist').innerHTML=ihtml
        document.querySelector("#lastReviewed").innerHTML=last_updat
        ct=0
        data.filelist.map(sign=>{
            document.querySelector("#id"+ct).addEventListener('click',(e)=>{
                currentword=sign.name
                play()
